...
val telegramAntiDisappearingMediaPatch = bytecodePatch(
    ...
    execute {
        ...
        // The old ChatActivity sendSecretMediaDelete/sendSecretMessageRead helpers were removed.
        // 12.10.5 executes the equivalent close callback inside SecretMediaViewer.e(ZZ).

        // SecretMediaViewer.e(ZZ) — prevent the obfuscated onClose Runnable from running — null out the onClose field to prevent destruction
            ...
            implementation!!.instructions
                .mapIndexedNotNull { i, insn ->
                    if (insn.opcode == Opcode.IGET_OBJECT &&
                        insn.toString().contains("n0")) i else null
                }
                .reversed()
                .forEach { idx ->
